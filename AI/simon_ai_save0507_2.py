...
#   in simulation, force to include obvious high win rate hand set, given shared cards
#   sort by judge_two, especially last round
#   different way to construct confident card set for different round:
#       round 0: pre-computed database
#       round 1: consider with known shared cards, iterate all possible; then simulate
#       round 2: iterate-simulate all
#       round 3: iterate all
...
class round2_iter():
	def __init__(self, last_card, _shared_cards, remain_card_comb, remain_card_comb_class):
		self.last_card = last_card
		self._shared_cards = _shared_cards
		self.remain_card_comb = remain_card_comb
		self.remain_card_comb_class = remain_card_comb_class
...
class ai():
	...
	def make_decision(self, state):
		...
		if len(bold_better_list) != 0:
			if round == 0:
				guess_card_list_full = pickle.load(open('/Users/Simon-CWG/Documents/quantresearch/Texaspoker/modules/texaspoker/AI/research/pair_score_sorted', 'rb'))
				for x in shared_cards:
				...
				while hand_level >= 3:
					guess_card_list_full.append(guess_cards(cards=remain_card_comb.pop(0)))
					pair = remain_card_comb[0]
					hand = Hand(list(pair) + shared_cards)
					hand_level = hand.level
				...
				remain_card_comb = list(combinations(remain_card, 2))
				remain_card_comb_class = list(map(lambda x: guess_cards_round2(cards = x), remain_card_comb))
				last_card_class = list(map(lambda x: round2_iter(x, shared_cards, remain_card_comb, remain_card_comb_class), remain_card))
				pool = multiprocessing.Pool()
				for last_card in remain_card:
					_shared_cards = shared_cards.copy()
					_shared_cards.append(last_card)
					_remain_card_comb = list(sorted(remain_card_comb, key=functools.cmp_to_key(rank_with_shared_card)))
					_remain_card_comb = list(reversed(_remain_card_comb))
					remain_card_comb_class = pool.map(round2_iter_func, last_card_class)
				def rank_to_win_rate(guess_cards_: guess_cards_round2):
					win_rate_list = list(map(lambda x: x**alive_players_num, guess_cards_.rank_list))
					guess_cards_.win_rate= sum(win_rate_list) / len(win_rate_list)
					guess_cards_.cards = list(guess_cards_.cards)
					return guess_cards_
				remain_card_comb_class = map(rank_to_win_rate, remain_card_comb_class)
				guess_card_list_full = sorted(remain_card_comb_class, key=(lambda x: x.win_rate))
...
if __name__ == '__main__':
	...
	for i in range(100):
		...
		state.sharedcards = cardset[2:6]
		state.currpos = 0
		state.player[1].totalbet = 40
		state.player[2].totalbet = 4000
		print('my cards')
